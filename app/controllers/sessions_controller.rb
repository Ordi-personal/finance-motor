class SessionsController < ApplicationController
  ...
  skip_authentication only: %i[index new create openid_connect failure post_logout destroy_current]
  ...
  # Allow external logout (e.g. from Fluxo) via GET
  def destroy_current
    if Current.session
    ...
    if params[:return_to].present?
      ...
      redirect_to new_session_path, notice: t("sessions.destroy.logout_successful")
    end
  end
